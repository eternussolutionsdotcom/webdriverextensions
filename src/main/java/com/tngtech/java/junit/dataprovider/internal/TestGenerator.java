...
import com.github.webdriverextensions.junitrunner.WebDriverRunner.BrowserConfiguration;
...
public class TestGenerator {
    ...
    private List<FrameworkMethod> explodeTestMethod(FrameworkMethod testMethod, Object data, DataProvider dataProvider) {
        ...
        //get browser context to support web driver runner 
        BrowserConfiguration browserContext = null;
        if(  testMethod instanceof WebDriverRunner.WebDriverFrameworkMethod  ){
        	browserContext = ((WebDriverRunner.WebDriverFrameworkMethod)testMethod).getBrowser();
        }

        int idx = 0;
        List<FrameworkMethod> result = new ArrayList<FrameworkMethod>();
        for (Object[] parameters : converted) {
        	DataProviderFrameworkMethod frameWorkMethod = new DataProviderFrameworkMethod(method, idx++, parameters, dataProvider.format());
        	
        	if( browserContext != null ){
        		frameWorkMethod.setBrowser(browserContext);
        	}
